...
user = users.getUser(user_id: str)  # Get user by their ID
...
user.energize(amt: int)     # Increase the user's energy by a specified amount
user.exhaust(amt: int)      # Decrease the user's energy by a specified amount
